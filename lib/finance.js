...
// Balances "as of" a boundary. Receivables/payables accrue on the RIDE
// COMPLETION date (COALESCE(ride.completed_at, row.created_at)) — NOT on the
// row's created_at. Backfilled rows are inserted long after the ride happened,
// and using created_at made July's closing "Driver owes HAPA" print 0.00.
const ACCRUAL_RECV=`COALESCE((SELECT rr.completed_at FROM ride_requests rr WHERE rr.id=dr.ride_id),dr.created_at)`;
const ACCRUAL_PAY=`COALESCE((SELECT rr.completed_at FROM ride_requests rr WHERE rr.id=dp.ride_id),dp.created_at)`;
async function openingBalances(db,driverId,start){
 const r=(await db.query(`SELECT
  ...
  (COALESCE((SELECT SUM(dr.amount) FROM driver_receivables dr WHERE dr.driver_user_id=$1 AND ${ACCRUAL_RECV}<$2),0)
   -COALESCE((SELECT SUM(si.amount) FROM driver_settlement_items si JOIN driver_settlements s ON s.id=si.settlement_id JOIN driver_receivables dr ON dr.id=si.item_id AND si.item_type='receivable' WHERE dr.driver_user_id=$1 AND ${ACCRUAL_RECV}<$2 AND s.completed_at<$2 AND s.status='completed'),0))::text AS owes_hapa,
  (COALESCE((SELECT SUM(dp.amount) FROM driver_payables dp WHERE dp.driver_user_id=$1 AND ${ACCRUAL_PAY}<$2),0)
   -COALESCE((SELECT SUM(si.amount) FROM driver_settlement_items si JOIN driver_settlements s ON s.id=si.settlement_id JOIN driver_payables dp ON dp.id=si.item_id AND si.item_type='payable' WHERE dp.driver_user_id=$1 AND ${ACCRUAL_PAY}<$2 AND s.completed_at<$2 AND s.status='completed'),0))::text AS hapa_owes`,
...
// Period movements between two boundaries, on the same accrual basis. Used for
// the loud reconciliation check: opening + movements must equal closing.
async function periodMovements(db,driverId,start,end){
 const r=(await db.query(`SELECT
  COALESCE((SELECT SUM(dr.amount) FROM driver_receivables dr WHERE dr.driver_user_id=$1 AND ${ACCRUAL_RECV}>=$2 AND ${ACCRUAL_RECV}<$3),0)::text AS recv_new,
  COALESCE((SELECT SUM(si.amount) FROM driver_settlement_items si JOIN driver_settlements s ON s.id=si.settlement_id JOIN driver_receivables dr ON dr.id=si.item_id AND si.item_type='receivable' WHERE dr.driver_user_id=$1 AND s.status='completed' AND (CASE WHEN ${ACCRUAL_RECV}<$2 THEN s.completed_at>=$2 AND s.completed_at<$3 ELSE s.completed_at<$3 END)),0)::text AS recv_settled,
  COALESCE((SELECT SUM(dp.amount) FROM driver_payables dp WHERE dp.driver_user_id=$1 AND ${ACCRUAL_PAY}>=$2 AND ${ACCRUAL_PAY}<$3),0)::text AS pay_new,
  COALESCE((SELECT SUM(si.amount) FROM driver_settlement_items si JOIN driver_settlements s ON s.id=si.settlement_id JOIN driver_payables dp ON dp.id=si.item_id AND si.item_type='payable' WHERE dp.driver_user_id=$1 AND s.status='completed' AND (CASE WHEN ${ACCRUAL_PAY}<$2 THEN s.completed_at>=$2 AND s.completed_at<$3 ELSE s.completed_at<$3 END)),0)::text AS pay_settled,
  COALESCE((SELECT SUM(CASE WHEN entry_type IN('topup','refund') THEN amount WHEN entry_type='commission_debit' THEN -amount ELSE amount END) FROM driver_commission_reserve_entries WHERE driver_user_id=$1 AND status='completed' AND created_at>=$2 AND created_at<$3),0)::text AS reserve_delta`,
  [driverId,start,end])).rows[0];
 return r;
}
async function generateStatement(db,driverId,year,month){
 const{start,end}=monthBounds(year,month);
 // Serialize concurrent (re)generation for the same driver/period: the
 // regenerate step is delete+reinsert, so parallel runs would collide on the
 // statement-items unique key. Advisory lock is released at COMMIT/ROLLBACK.
 await db.query(`SELECT pg_advisory_xact_lock(hashtext('hapa-stmt:'||$1||':'||$2||':'||$3))`,[String(driverId),String(year),String(month)]);
 ...
 const rides=(await db.query(`SELECT r.id,r.ride_reference,r.completed_at,r.payment_method,r.pickup_address AS pickup_label,r.dest_address AS dest_label,r.status,
   r.created_at AS requested_at,r.assigned_at AS accepted_at,r.started_at,
   CASE WHEN r.started_at IS NOT NULL AND r.completed_at IS NOT NULL THEN ROUND(EXTRACT(EPOCH FROM r.completed_at-r.started_at))::int END AS actual_duration_s,
   fq.distance_m,dv.registration_number AS vehicle_registration,ga.name AS zone_name,
   ...
   (SELECT amount::text FROM driver_receivables WHERE ride_id=r.id LIMIT 1) AS receivable_amount,
   (SELECT outstanding::text FROM driver_receivables WHERE ride_id=r.id LIMIT 1) AS receivable_outstanding,
   (SELECT status FROM driver_receivables WHERE ride_id=r.id LIMIT 1) AS receivable_status,
   (SELECT reference FROM driver_payables WHERE ride_id=r.id AND source='mpesa_fare' LIMIT 1) AS payable_reference,
   (SELECT amount::text FROM driver_payables WHERE ride_id=r.id AND source='mpesa_fare' LIMIT 1) AS payable_amount,
   (SELECT status FROM driver_payables WHERE ride_id=r.id AND source='mpesa_fare' LIMIT 1) AS payable_status,
  ...
  LEFT JOIN fare_quotes fq ON fq.id=r.quote_id LEFT JOIN driver_vehicles dv ON dv.id=r.vehicle_id LEFT JOIN geo_areas ga ON ga.id=r.zone_id
  WHERE r.driver_user_id=$1 AND r.completed_at>=$2 AND r.completed_at<$3 ORDER BY r.completed_at`,[driverId,start,end])).rows;
 const cancelled=Number((await db.query(`SELECT COUNT(*) FROM ride_requests WHERE driver_user_id=$1 AND status IN('rider_cancelled','driver_cancelled','cancelled') AND cancelled_at>=$2 AND cancelled_at<$3`,[driverId,start,end])).rows[0].count);
 ...
 const sum={rides:rides.length,cancelled_rides:cancelled,gross:0,commission:0,net:0,cash_gross:0,cash_net:0,cash_commission:0,mpesa_gross:0,mpesa_net:0,mpesa_commission:0,tips_mpesa:0,tips_cash_declared:0,reserve_topups:0,reserve_debits:0,settled_to_hapa:0,settled_to_driver:0,refunds:0,adjustments:0};
 ...
 // ── Loud reconciliation: opening + period movements MUST equal closing. ─────
 // A statement that does not balance must never produce a PDF.
 const mv=await periodMovements(db,driverId,start,end);
 // Test-only fault injection (never in production): forces a reconciliation
 // failure for one driver so the alert workflow can be exercised end-to-end.
 // Real imbalances cannot be produced through public APIs by construction.
 // Value may be a driver user id, or "domain:<email-domain>" to match any
 // synthetic test driver whose email ends with that domain (lets tests create
 // the faulty driver after the server has started).
 let faultC=0;
 const faultV=process.env.NODE_ENV!=='production'?process.env.FINANCE_FAULT_INJECT_DRIVER:null;
 if(faultV){
  if(faultV.startsWith('domain:')){
   const u=(await db.query(`SELECT email FROM users WHERE id=$1`,[driverId])).rows[0];
   if(u&&String(u.email||'').toLowerCase().endsWith('@'+faultV.slice(7).toLowerCase()))faultC=100;
  }else if(String(faultV)===String(driverId))faultC=100;
 }
 const expOwes=cents(opening.driver_owes_hapa)+cents(mv.recv_new)-cents(mv.recv_settled)+faultC;
 const expOwed=cents(opening.hapa_owes_driver)+cents(mv.pay_new)-cents(mv.pay_settled);
 const expRes=cents(opening.reserve)+cents(mv.reserve_delta);
 if(expOwes!==cents(closing.driver_owes_hapa)||expOwed!==cents(closing.hapa_owes_driver)||expRes!==cents(closing.reserve))
  throw Object.assign(new Error(`Statement reconciliation failed for driver ${driverId} ${year}-${p2(month)}: `+
   `owes ${kes(expOwes)} vs ${closing.driver_owes_hapa}, owed ${kes(expOwed)} vs ${closing.hapa_owes_driver}, reserve ${kes(expRes)} vs ${closing.reserve}`),
   {code:500,reconciliation:true,statementId:st.id,statementReference:st.reference,detail:{
    equation:'opening balance + period movements = closing balance',
    stage:'closing_balance_reconciliation',
    expected:{driver_owes_hapa:kes(expOwes),hapa_owes_driver:kes(expOwed),reserve:kes(expRes)},
    actual:{driver_owes_hapa:closing.driver_owes_hapa,hapa_owes_driver:closing.hapa_owes_driver,reserve:closing.reserve},
    difference_kes:{driver_owes_hapa:kes(expOwes-cents(closing.driver_owes_hapa)),hapa_owes_driver:kes(expOwed-cents(closing.hapa_owes_driver)),reserve:kes(expRes-cents(closing.reserve))},
    opening,movements:{receivables_new:mv.recv_new,receivables_settled:mv.recv_settled,payables_new:mv.pay_new,payables_settled:mv.pay_settled,reserve_delta:mv.reserve_delta}
   }});
 sum.amount_settled=sum.settled_to_hapa;
 sum.outstanding=cents(closing.driver_owes_hapa);
 const summary={};for(const k of Object.keys(sum))summary[k]=(k==='rides'||k==='cancelled_rides')?sum[k]:kes(sum[k]);
 const meta={
  zones:[...new Set(rides.map(r=>r.zone_name).filter(Boolean))],
  vehicles:[...new Set(rides.map(r=>r.vehicle_registration).filter(Boolean))],
  driver_account_reference:'DRV-'+String(driverId).slice(0,8).toUpperCase(),
  timezone:'Africa/Nairobi',
  reconciliation:{opening,movements:{receivables_new:mv.recv_new,receivables_settled:mv.recv_settled,payables_new:mv.pay_new,payables_settled:mv.pay_settled,reserve_delta:mv.reserve_delta},closing,balanced:true}
 };
 const status=st.status==='open'&&new Date()>=end?'ready_for_review':st.status;
 st=(await db.query(`UPDATE driver_monthly_statements SET opening=$2,closing=$3,summary=$4,meta=$5,status=$6,issued_at=NOW(),updated_at=NOW() WHERE id=$1 RETURNING *`,
  [st.id,JSON.stringify(opening),JSON.stringify(closing),JSON.stringify(summary),JSON.stringify(meta),status])).rows[0];
 return st;
}
