...
app.get('/api/owner/users',auth,owner,async(req,res)=>{try{const r=await q(`SELECT u.id,u.name,u.email,u.phone,u.role,u.status,u.email_verified,u.phone_verified,u.capabilities,u.created_at,u.profile_photo_url,count(ml.id)::int AS listing_count FROM users u LEFT JOIN marketplace_listings ml ON ml.seller_id=u.id GROUP BY u.id ORDER BY u.created_at DESC`);res.json(r.rows)}catch(e){console.error(e);res.status(500).json({error:'Server error'})}});
app.get('/api/owner/users/:id',auth,owner,async(req,res)=>{try{const r=await q(`SELECT u.id,u.name,u.email,u.phone,u.role,u.status,u.email_verified,u.phone_verified,u.capabilities,u.created_at,u.profile_photo_url,count(ml.id) FILTER(WHERE ml.status='active')::int AS active_listings,count(ml.id) FILTER(WHERE ml.status='hidden')::int AS hidden_listings,count(ml.id) FILTER(WHERE ml.status='removed')::int AS removed_listings,count(ml.id)::int AS total_listings FROM users u LEFT JOIN marketplace_listings ml ON ml.seller_id=u.id WHERE u.id=$1 GROUP BY u.id`,[req.params.id]);if(!r.rowCount)return res.status(404).json({error:'User not found'});res.json(r.rows[0])}catch(e){console.error(e);res.status(500).json({error:'Server error'})}});
app.get('/api/owner/users/:id/listings',auth,owner,async(req,res)=>{try{const r=await q(`SELECT id,title,price,category,condition,location,status,created_at,jsonb_array_length(images)::int AS image_count,images->0 AS main_image,description FROM marketplace_listings WHERE seller_id=$1 ORDER BY created_at DESC`,[req.params.id]);res.json(r.rows)}catch(e){console.error(e);res.status(500).json({error:'Server error'})}});
app.patch('/api/owner/users/:id/status',auth,owner,async(req,res)=>{try{const s=req.body.status;if(!['active','rejected','blocked'].includes(s))return res.status(400).json({error:'Invalid status'});const chk=await q('SELECT role FROM users WHERE id=$1',[req.params.id]);if(!chk.rowCount)return res.status(404).json({error:'User not found'});if(chk.rows[0].role==='owner')return res.status(403).json({error:'Cannot modify owner account'});const r=await q('UPDATE users SET status=$2 WHERE id=$1 RETURNING id,name,email,role,status',[req.params.id,s]);res.json(r.rows[0])}catch(e){console.error(e);res.status(500).json({error:'Server error'})}});
app.get('/api/owner/listings/:id',auth,owner,async(req,res)=>{try{const r=await q(`SELECT ml.*,u.name AS seller_name,u.email AS seller_email,u.phone AS seller_phone FROM marketplace_listings ml JOIN users u ON u.id=ml.seller_id WHERE ml.id=$1`,[req.params.id]);if(!r.rowCount)return res.status(404).json({error:'Listing not found'});res.json(r.rows[0])}catch(e){console.error(e);res.status(500).json({error:'Server error'})}});
app.patch('/api/owner/listings/:id/status',auth,owner,async(req,res)=>{try{const s=req.body.status;if(!['active','hidden','removed'].includes(s))return res.status(400).json({error:'Invalid status'});const r=await q('UPDATE marketplace_listings SET status=$2 WHERE id=$1 RETURNING *',[req.params.id,s]);if(!r.rowCount)return res.status(404).json({error:'Listing not found'});res.json(r.rows[0])}catch(e){console.error(e);res.status(500).json({error:'Server error'})}});
